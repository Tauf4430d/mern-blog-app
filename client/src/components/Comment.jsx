  ...
  return (
    <div className='flex p-4 border-b dark:border-gray-600 text-sm'>
      <div className='flex-shrink-0 mr-3 '>
        <img className='w-10 h-10 rounded-full bg-gray-200' src={user.profilePhoto} />
      </div>
      <div className='flex-1'>
        <div className="flex items-center mb-1">
          <span className='font-bold mr-1 text-xs trunacte '>{user ? `@${user.username}` : 'anonymous user'}</span>
          ...
          isEditing ? (
            <>
              ...
              <div className='flex justify-end gap-2 text-xs'>
                <Button className='sm' type='button' gradientDuoTone='purpleToBlue' onClick={handleSave}>
                  Save
                </Button>
                <Button className='sm' type='button' gradientDuoTone='purpleToBlue' outline onClick={() => setIsEditing(false)}>
                  Cancel 
                </Button>
              </div>
            </>
